...
** CUD + 단순 R : JPA
** 복합적인 R
*** JPA를 써도 분리된 패턴으로 구현하고 있을 것임
*** Spring JDBC로도 쓸만함 : 단순한 쿼리 실행기, 확장 가능
* 하나의 프레임워크로 통일한다면 Spring Data JDBC도 고려해볼만함
** Entity 단위의 접근이 어려운 부분은 Spring JDBC처럼 사용하면 됨.

=== 아키텍처 전락
[incremental="true"]
* AGGREGATE가 단위로 정리하기 어려운 부분을 별도의 시스템으로 분리하는 것도 고려해볼만함.
